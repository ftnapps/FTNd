...
int CountNGroup(void)
{
	...
	snprintf(ffile, PATH_MAX, "%s/etc/ngroups.data", getenv("FTND_ROOT"));
...
int OpenNGroup(void)
{
	...
	snprintf(fnin,  PATH_MAX, "%s/etc/ngroups.data", getenv("FTND_ROOT"));
	snprintf(fnout, PATH_MAX, "%s/etc/ngroups.temp", getenv("FTND_ROOT"));
...
void CloseNGroup(int force)
{
	...
	snprintf(fin,  PATH_MAX, "%s/etc/ngroups.data", getenv("FTND_ROOT"));
	snprintf(fout, PATH_MAX, "%s/etc/ngroups.temp", getenv("FTND_ROOT"));
...
int AppendNGroup(void)
{
	...
	snprintf(ffile, PATH_MAX, "%s/etc/ngroups.temp", getenv("FTND_ROOT"));
	if ((fil = fopen(ffile, "a")) != NULL) {
		memset(&ngroup, 0, sizeof(ngroup));
		fwrite(&ngroup, sizeof(ngroup), 1, fil);
		fclose(fil);
...
void NgScreen(void)
{
	...
	ftnd_mvprintw( 5, 6, "9.1 EDIT NEWFILES GROUP");
	set_color(CYAN, BLACK);
	ftnd_mvprintw( 7, 6, "1.  Name");
	ftnd_mvprintw( 8, 6, "2.  Comment");
	ftnd_mvprintw( 9, 6, "3.  Active");
