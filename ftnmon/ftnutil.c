 ...
 *
 * This FTNd is free software; you can redistribute it and/or modify it
...
{
    ftnd_mvprintw(y, x, padleft(str, length, fill));
}
...
void show_date(int fg, int bg, int y, int x)
{
    ...
    if (now != lasttime) {
        ...
        p = ctime(&now);
        Striplf(p);
        ftnd_mvprintw(1, cols - 36, (char *)"%s TZUTC %s", p, gmtoffset(now)); 
        p = asctime(gmtime(&now));
        Striplf(p);
        ftnd_mvprintw(2, cols - 36, (char *)"%s UTC", p);
        ...
        if (strncmp(buf, "100:0;", 6) == 0) {
            ...
            set_color(WHITE, RED);
            ftnd_mvprintw(2,cols - 6, (char *)" Down ");
        } else {
            set_color(WHITE, BLUE);
            ftnd_mvprintw(2,cols - 6, (char *)" Free ");
        }
        bbs_free = TRUE;
    } else {
        set_color(WHITE, RED);
        ftnd_mvprintw(2,cols - 6, (char *)" Busy ");
        bbs_free = FALSE;
    }
    ...
    if (strcmp(buf, "100:0;") == 0) {
        ...
        if (strcmp(page, "1")) {
            set_color(RED, BLACK);
            ftnd_mvprintw(3, 1, "   Old page (%s) %-60s", pid, reason);
        if ((now % 10) == 0)	/* Every 10 seconds */
            putchar(7);
        } else {
            set_color(LIGHTRED, BLACK);
            ftnd_mvprintw(3, 1, " Sysop page (%s) %-60s", pid, reason);
...
{
    ftnd_mvprintw(y, (cols / 2) - (strlen(s) / 2), s);
}
...
void screen_start(char *name)
{
    ...
    set_color(WHITE, BLUE);
    ftnd_locate(1, 1);
    printf((char *)"%s for FTNd version %s", name, VERSION);  
    set_color(YELLOW, BLUE);
    ftnd_locate(2, 1);
...
void working(int txno, int y, int x)
{
    ...
    switch (txno) {
        case 0: ftnd_mvprintw(4, cols - 14, (char *)"             ");
            break;
        case 1: ftnd_mvprintw(4, cols - 14, (char *)"Working . . .");
            break;
        case 2:	ftnd_mvprintw(4, cols - 14, (char *)">>> ERROR <<<");
            for (i = 1; i <= 5; i++) {
                putchar(7);
                fflush(stdout);
                msleep(150);
            ...
            msleep(550);
            break;
        case 3: ftnd_mvprintw(4, cols - 14, (char *)"Form inserted");
            putchar(7);
            fflush(stdout);
            sleep(1);
            break;
        case 4: ftnd_mvprintw(4, cols - 14, (char *)"Form deleted ");
            putchar(7);
            fflush(stdout);
            sleep(1);
            break;
