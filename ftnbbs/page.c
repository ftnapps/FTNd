...
void Page_Sysop(char *String)
{
	 ...
	 * chatserver, the sysop can only use ftnmon.
	 */
    ...
    if (socket_send(buf)) {
	Syslog('+', "Failed to send message to ftntask");
	free(Reason);
	return;
    }
...
void PageReason()
{
    ...
    snprintf(temp, PATH_MAX, "%s/share/int/txtfiles/%s/page.asc", getenv("FTND_ROOT"), CFG.deflang);
