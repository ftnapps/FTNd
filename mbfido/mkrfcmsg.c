/*****************************************************************************
 *
 * File ..................: mbfido/mkrfcmsg.c
 * Purpose ...............: Import a email or news
...
char *rfcmsgid(char *msgid, faddr *bestaka)
{
	...
	if (id != 0L) {
		...
		sprintf(buf,"<%lu@%s.ftn>",(unsigned long)sequencer(), ascinode(bestaka,0x1f));
	}
	tidy_faddr(ta);
	if (r) 
		*r='\n';
...
/*
 * Lines to send, terminated with a newline character.
 */
...
/*
 * Import gated email into users email box or import news article
 * on the news server.
 *
 *  0 - All seems well.
 ...
 *
 */
int mkrfcmsg(faddr *f, faddr *t, char *subj, char *origline, time_t mdate, int flags, FILE *pkt, off_t endoff, int ftn_from)
{
	...
	if (newsmode) {
		...
		for (qmsg = kmsg; qmsg; qmsg = qmsg->next)
			if (strcasecmp(qmsg->key, "SPTH") == 0)
				fill_list(&ftnpath, qmsg->val, &rlist, FALSE);
		for (qmsg = kmsg; qmsg; qmsg = qmsg->next)
